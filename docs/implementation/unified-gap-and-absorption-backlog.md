...
2. tratar `MB-160` a `MB-187` como baseline fechado e executar `MB-188` como
   medicao longitudinal de ganho e regressao de skills, workflows e memoria
   revisados, sem converter score em promocao.
...
- `MB-160` abriu a fila maior `MB-161` a `MB-174`, priorizando memoria causal,
  ...
  `MB-189`; `MB-177` a `MB-187` tambem foram fechados e `MB-188` e o unico item `ready`;
