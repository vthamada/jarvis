...
2. tratar `MB-160` a `MB-186` como baseline fechado e executar `MB-187` como
   fila humana de revisao, consolidacao, archive e expiracao de memoria, sem
   scheduler autonomo.
...
- `MB-160` abriu a fila maior `MB-161` a `MB-174`, priorizando memoria causal,
  ...
  `MB-189`; `MB-177` a `MB-186` tambem foram fechados e `MB-187` e o unico item `ready`;
