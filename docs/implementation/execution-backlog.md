...
- `impacto_no_baseline`: pattern evidence revisada agora pode gerar delta bounded e candidata versionada inativa; revisao do pattern nao aprova a candidata nem altera o registry ativo.
- `evidencia_de_fechamento`: contratos/schemas `WorkflowEvolution*`, proposta e verificacao de review persistido no `evolution-lab`, builder fail-closed e E2E memoria-pattern-review-delta-side-registry preservam todas as flags de autoridade falsas.
...
- `impacto_no_baseline`: candidata de workflow agora possui comparacao offline equivalente, com dimensoes fixas, verificacao do contrato versionado e regressao fail-closed; resultado verde continua manual-gate-only.
- `evidencia_de_fechamento`: contratos/schemas `WorkflowVariantEval*`, avaliacao por caso no `evolution-lab`, agregacao no `observability-service`, runner em `tools/workflow_variant_eval.py` e testes pass/regressao/authority preservam registry ativo e `promotion_authorized=false`.
...
- `impacto_no_baseline`: candidata de workflow agora recebe proposta/review proprios, eval equivalente e rollback manual verificado antes de entrar no promotion gate existente; gate verde continua pendente de decisao humana separada.
- `evidencia_de_fechamento`: `WorkflowRollbackPlanContract`, proposta `workflow_candidate`, extensao do checklist/gate, `tools/workflow_release_gate.py` e E2E pass/block comprovam `manual_only`, candidata inativa, registry ativo imutavel e `promotion_authorized=false`.
...
- `impacto_no_baseline`: telemetria canonica e reviewed guidance expirado agora geram candidatos humanos de consolidacao/archive/expiracao; approve e rollback persistem somente disposicao revisada, sem executar manutencao.
- `evidencia_de_fechamento`: contratos/schemas `MemoryLifecycle*`, persistencia SQLite/PostgreSQL, assessment fail-closed, comandos `memory-review-*` e E2E de missao comprovam evidencia, rollback e fontes imutaveis.
...
- `modo_de_raciocinio_recomendado`: `high`
- `modelo_recomendado`: `gpt-5.3-codex`
- `impacto_no_baseline`: `jarvis-console doctor` diagnostica runtime Python, imports, diretorio local, stores SQLite em modo read-only, sincronizacao do backlog, fronteira de governanca e disponibilidade do gate; o resultado text/JSON distingue healthy, degraded e failed sem reparo implicito.
- `evidencia_de_fechamento`: testes unitarios e CLI comprovam Core lazy, ausencia de criacao/mutacao de stores, warning para runtime nao inicializado, falha fechada para SQLite invalido e drift documental, redacao de paths e exit codes `0/1`; `docs/operations/cli-doctor.md` registra a operacao.
...
- `modo_de_raciocinio_recomendado`: `high`
- `modelo_recomendado`: `gpt-5.3-codex`
- `impacto_no_baseline`: work items governados possuem estado estruturado canonico com prioridade `p0..p3`, dependencias, blockers e blocking state; SQLite/PostgreSQL preservam o grafo entre sessoes, e o console expoe ordem topologica read-only sem executar ou agendar trabalho.
- `evidencia_de_fechamento`: contratos/schemas, politica compartilhada, governanca fail-closed, memoria, orquestracao, workspace e CLI cobrem create/update, ciclo, dependencia desconhecida, bloqueio explicito, readiness apos completion e E2E cross-session; `docs/operations/governed-work-item-queue.md` registra a operacao.
...
- `modo_de_raciocinio_recomendado`: `high`
- `modelo_recomendado`: `gpt-5.3-codex`
- `impacto_no_baseline`: loops abertos agora possuem registry read-only, refs deterministicas, freshness e blockers; a retomada exige selecao explicita, revalidacao pelo Core, persistencia canonica protegida contra corrida e sintese de next action, sem executar ou agendar trabalho.
- `evidencia_de_fechamento`: contratos/schemas, policy compartilhada, planning, governance, memory SQLite/PostgreSQL, operational projection, orchestrator, eventos e comandos `open-loops`/`resume-loop` cobrem o fluxo; testes unitarios e E2E em tres sessoes validam resume, stale/conflict, repeticao, corrida de estado, persistencia e ausencia de dispatch; `docs/operations/governed-open-loop-resume.md` registra a operacao.
