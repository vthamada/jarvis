...
- `MB-175` repriorizou o slice para pattern evidence, skills e workflows governados; `MB-176` fechou a deteccao auditavel, `MB-177` o registry inativo e `MB-178` o miner bounded; `MB-179` conecta candidata a review/sandbox sem ativacao.
...
- `MB-160` foi fechado como repriorizacao pos-`MB-159` a partir de
  ...
  fechado, `MB-177` e `MB-178` tambem foram fechados e `MB-179` e o unico item tecnico
  `ready`.
