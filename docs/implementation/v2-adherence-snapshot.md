...
- `MB-175` repriorizou o slice para pattern evidence, skills e workflows governados; `MB-176` a `MB-183` fecharam a cadeia de skill, versionamento imutavel, derivacao revisada e eval offline sem regressao; `MB-184` inicia a integracao aos gates humanos de promocao e rollback.
...
- `MB-160` foi fechado como repriorizacao pos-`MB-159` a partir de
  ...
  fechado, `MB-177` a `MB-183` tambem foram fechados e `MB-184` e o unico item tecnico
  `ready`.
