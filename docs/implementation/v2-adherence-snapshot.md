...
- `MB-175` repriorizou o slice para pattern evidence, skills e workflows governados; `MB-176` a `MB-184` fecharam a cadeia de skill e workflow ate gate/rollback manual, sem promocao automatica; `MB-185` inicia evidencia de adaptacao de routing.
...
- `MB-160` foi fechado como repriorizacao pos-`MB-159` a partir de
  ...
  fechado, `MB-177` a `MB-184` tambem foram fechados e `MB-185` e o unico item tecnico
  `ready`.
