...
- `MB-174` consolidou maturidade por capacidade, gate/testes, guardrails documentais e sincronismo da fila em relatorio/console read-only, com historico JSON e release autonomo sempre bloqueado.
- `MB-175` repriorizou o slice para pattern evidence, skills e workflows governados; `MB-176` a `MB-184` fecharam a cadeia de skill e workflow ate gate/rollback manual; `MB-185` adicionou evidencia bounded de routing, `MB-186` fechou prioridade/conflito/non-use causal, `MB-187` adicionou revisao humana persistida de lifecycle sem executar manutencao, `MB-188` fechou medicao longitudinal read-only e `MB-189` integrou essa evidencia ao readiness dashboard com validacao fail-closed.
- `MB-190` abriu o Daily Operator Loop em `MB-191` a `MB-200`: registry,
  runtime/output e doctor estabilizam o CLI antes de workspace diario, work
  items, artefatos, resume, metricas e fechamento.
- `MB-191` removeu o branch chain de dispatch e tornou metadata/handler/mode de
  cada comando validaveis, mantendo os mesmos paths governados.
- `MB-192` adicionou JSON bounded para reads/reports, redaction e exit semantics
  sem habilitar mutacao ou novo poder pelo formato de output.
- `MB-193` a `MB-197` fecharam preflight local, workspace multi-missao, grafo
  governado de work items, linhagem canonica de artefatos e retomada explicita
  de open loops; toda continuidade cruza Core, evidencia e memoria canonica,
  enquanto scheduler, dispatch no resume e autonomia ampla permanecem falsos.
...
- `MB-160` foi fechado como repriorizacao pos-`MB-159` a partir de
  ...
  seguinte; `MB-191` a `MB-197` foram fechados e `MB-198` e o unico item
  tecnico `ready`.
- `MB-161` adicionou anchors de evidencia para memoria semantica:
  ...
  planning, synthesis, eventos, observabilidade e console/dashboard.
...
- `MB-164` formalizou o `autonomy_ladder` como contrato runtime: requests,
  plano, governanca, dispatch e eventos carregam nivel solicitado, maximo,
  efetivo, confirmacao humana e bloqueios de autopromocao/core mutation, ainda
  sem enforcement amplo.
- `MB-165` aplicou enforcement minimo do `autonomy_ladder`: governanca defere
  capability acima do limite, operacional bloqueia dispatch indevido sem gerar
  artefato, e observabilidade/console mostram o limite aplicado.
- `MB-166` definiu o checklist sandbox-to-release executavel: proposta e revisao
  produzem escopo, evidencias, testes, rollback, gates e blockers auditaveis,
  sem transformar checklist completo em permissao de promocao.
- `MB-167` tornou o promotion gate executavel e observavel: decisao canonica,
  validacao defensiva, evento auditavel e release baseline pass/block preservam
  decisao humana final e bloqueiam promocao quando falta qualquer gate.
- `MB-168` consolidou o cockpit textual: objetivo, work items, artefatos,
  revisoes, memoria, autonomia, promotion gate e proximas decisoes agora
  aparecem em uma unica leitura read-only, sem executar acao.
