...
- `MB-175` repriorizou o slice para pattern evidence, skills e workflows governados; `MB-176` a `MB-180` fecharam evidencia, registry, miner, review/sandbox e leitura do operador; `MB-181` inicia versionamento de workflow sem mutar o registry ativo.
...
- `MB-160` foi fechado como repriorizacao pos-`MB-159` a partir de
  ...
  fechado, `MB-177` a `MB-180` tambem foram fechados e `MB-181` e o unico item tecnico
  `ready`.
