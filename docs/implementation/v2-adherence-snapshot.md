...
- `MB-175` repriorizou o slice para pattern evidence, skills e workflows governados; `MB-176` a `MB-184` fecharam a cadeia de skill e workflow ate gate/rollback manual; `MB-185` adicionou evidencia bounded de routing, `MB-186` fechou prioridade/conflito/non-use causal, `MB-187` adicionou revisao humana persistida de lifecycle sem executar manutencao e `MB-188` fechou medicao longitudinal read-only; `MB-189` fecha readiness e docs do slice.
