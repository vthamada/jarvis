...
| `observabilidade, validação e evals` | `44`, `55`, `66`, `233`, `301` a `311`, `329` a `342` | `runtime parcial` | `corrigir agora` | `observability-service`, `tools/internal_pilot_report.py`, `tools/compare_orchestrator_paths.py`, `evolution-lab`, `tools/close_alignment_cycle.py` | os sinais por eixo já fecharam formalmente o `v2-alignment-cycle`, mas agora precisam virar gate contínuo do próximo corte | usar os cinco eixos e o `axis_gate_status` como regra permanente de comparação, piloto e promoção do próximo corte | `próximo corte do v2` |
