...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `identidade, missão, princípios e filosofia` | `2` a `8`, `26`, `231` | `runtime parcial` | `corrigir agora` | `identity-engine`, `orchestrator-service`, `governance-service`, `observability-service` | a identidade já é auditável no fluxo principal, mas ainda não opera como gate transversal de fechamento por eixo | usar `identity_alignment_status` como critério explícito de observabilidade, evals e promoção do ciclo | `v2-alignment-cycle`, Sprint 5 |
...
| `mentes` | `10`, `19`, `21`, `38`, `50` | `runtime parcial` | `corrigir agora` | `shared/mind_registry.py`, `engines/cognitive-engine`, `orchestrator-service`, `observability-service` | o registry já governa a arbitragem do runtime principal, mas ainda falta usar aderência cognitiva como gate explícito do ciclo | usar `mind_alignment_status` como critério de observabilidade, evals e fechamento do alinhamento | `v2-alignment-cycle`, Sprint 5 |
...
| `memórias` | `14`, `23`, `40`, `51`, `83`, `232` | `runtime parcial` | `corrigir agora` | `shared/memory_registry.py`, `memory-service`, `orchestrator-service`, `observability-service` | as classes já governam recovery e compartilhamento, mas ainda falta usá-las como gate explícito de aderência do ciclo | usar `memory_alignment_status` como critério de observabilidade, evals e promoção ou contenção do recorte atual | `v2-alignment-cycle`, Sprint 5 |
...
| `observabilidade, validação e evals` | `44`, `55`, `66`, `233`, `301` a `311`, `329` a `342` | `runtime parcial` | `corrigir agora` | `observability-service`, `tools/internal_pilot_report.py`, `tools/compare_orchestrator_paths.py`, `evolution-lab` | os sinais por eixo já viraram gate explícito, mas ainda falta usá-los para fechar formalmente o `v2-alignment-cycle` | usar os cinco eixos e o `axis_gate_status` para sustentar a decisão final da Sprint 6 | `v2-alignment-cycle`, Sprint 6 |
