...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `identidade, missão, princípios e filosofia` | `2` a `8`, `26`, `231` | `runtime parcial` | `corrigir agora` | `identity-engine`, `orchestrator-service`, `governance-service`, `observability-service` | a identidade já é auditável no fluxo principal e já compõe o fechamento formal do ciclo por eixo | manter `identity_alignment_status` como gate transversal obrigatório de promoção no próximo corte | `próximo corte do v2` |
...
| `mentes` | `10`, `19`, `21`, `38`, `50` | `runtime parcial` | `corrigir agora` | `shared/mind_registry.py`, `engines/cognitive-engine`, `orchestrator-service`, `observability-service` | o registry já governa a arbitragem do runtime principal e agora já sustenta o fechamento formal do alinhamento | manter `mind_alignment_status` como gate contínuo de observabilidade, evals e promoção do próximo corte | `próximo corte do v2` |
| `domínios` | `11`, `12`, `13`, `22`, `39`, `52`, `232` | `runtime parcial` | `corrigir agora` | `knowledge-service`, `knowledge/curated/v1_corpus.json`, `knowledge/curated/domain_registry.json`, `cognitive-engine`, `specialist-engine` | o mapa canônico já está no registry, mas o runtime ainda precisa operar com eventos e arbitragem totalmente derivados dele | tornar o registry a fonte soberana de roteamento, corpus ativo e rotas guiadas por domínio | `v2-sovereign-alignment-cut`, Sprints 2 e 5 |
| `memórias` | `14`, `23`, `40`, `51`, `83`, `232` | `runtime parcial` | `corrigir agora` | `shared/memory_registry.py`, `memory-service`, `orchestrator-service`, `observability-service` | as classes já governam recovery e compartilhamento e agora já entram no fechamento formal do ciclo | manter `memory_alignment_status` como gate obrigatório enquanto surgem consumidores canônicos de memória mais rica | `próximo corte do v2` |
...
| `observabilidade, validação e evals` | `44`, `55`, `66`, `233`, `301` a `311`, `329` a `342` | `runtime parcial` | `corrigir agora` | `observability-service`, `tools/internal_pilot_report.py`, `tools/compare_orchestrator_paths.py`, `evolution-lab`, `tools/archive/close_alignment_cycle.py` | os sinais por eixo já fecharam formalmente o `v2-alignment-cycle`, mas agora precisam virar gate contínuo do próximo corte | usar os cinco eixos e o `axis_gate_status` como regra permanente de comparação, piloto e promoção do próximo corte | `próximo corte do v2` |
...
| `implementação, operação, release e incidentes` | `286` a `350` | `runtime parcial` | `corrigir agora` | `docs/implementation`, `docs/operations`, `tools/archive/close_stateful_runtime_cycle.py`, `tools/validate_baseline.py` | a operação está madura para o baseline, mas o mestre ainda não é usado como régua contínua de cada ciclo | recalibrar o `v2` para declarar eixo, lacuna e não-cobertura por sprint | `v2`, imediato |
...
- usar `identity_alignment_status` para transformar princípios identitários em critério explícito de observabilidade, evals e fechamento do ciclo.
...
- Sprint 6 fechou formalmente o `v2-alignment-cycle`, classificando backlog entre `next_cut`, `deferred` e `vision`;
- o próximo corte do `v2` passa a nascer já subordinado a esses gates por eixo.
